<template>
  <div class="dropdown">
    ...
    <div class="dropdown__form-wrapper" role="combobox" :aria-expanded="showOptions" :aria-owns="listboxId" aria-haspopup="listbox" :id="comboboxId">
      <div class="dropdown__header" :class="{ 'is-active': !showOptions }">
        <button class="dropdown__toggle-button" ref="toggleButton" @click="openDropdown" v-on:keydown.enter.prevent="" v-on:keyup.enter.prevent="openDropdown" v-on:keyup.space.prevent="openDropdown" v-on:keyup.down.prevent="openDropdown" v-on:keyup.up.prevent="openDropdown" :aria-label="buttonLabel">
      ...
      <transition name="grow">
        <ul :id="listboxId" class="dropdown__options" v-if="showOptions" tabindex="-1" role="listbox" :aria-labelledby="labelId">
          <li class="dropdown__options-item" :class="{ 'dropdown__options-items--selected': index === chosenIndex }" :id="item.Value" v-for="(item, index) in filteredOptions" :key="item.Value" tabindex="0" :ref="'item-' + index" @click="setValue(item)">
            {{ item.value }}
          </li>
        </ul>
      </transition>
...
export default {
  ...
  methods: {
    ...
    goToNext() {
      ...
      if (this.optionsLength === 0 || this.chosenIndex === this.optionsLength || this.chosenIndex === null) {
        this.chosenIndex = 0;
      } else {
        this.chosenIndex++;
      }
    ...
    goToPrevious() {
      ...
      if (this.optionsLength === 0 || this.chosenIndex === 0 || this.chosenIndex === null) {
